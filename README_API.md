...
> 2. 以下API是参考API，未使用Restful风格，目前只使用Get, Post。

## 1 前后端接口规范

### 1.1 请求格式

项目没有采用RESTful风格的接口，而是定义具体语义的接口。
...
访问受保护商场API采用自定义`X-Librecmall-Token`头部

1. 商城前端访问商城后端登录API`/wx/auth/login`
...
3. 请求受保护API则，则采用自定义头部携带此token
        ...
        X-Librecmall-Token: eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJ0aGlzIGlzIGxpdGVtYWxsIHRva2VuIiwiYXVkIjoiTUlOSUFQUCIsImlzcyI6IkxJVEVNQUxMIiwiZXhwIjoxNTU3MzM2ODU0LCJ1c2VySWQiOjIsImlhdCI6MTU1NzMyOTY1NH0.JY1-cqOnmi-CVjFohZMqK2iAdAH4O6CKj0Cqd5tMF3M

#### 1.4.3 管理后台自定义Header

访问受保护管理后台API则是自定义`X-Librecmall-Admin-Token`头部。
...
3. 请求受保护API时，则采用自定义头部携带此token
        ...
        GET http://localhost:8080/wx/address/list
        X-Librecmall-Admin-Token: f2dbcae8-6e25-4f8e-bc58-aa81d512c952
...
接下来会分别从用户层面和管理员层面构建商城API服务和管理后台API服务。
...
注意：
> 访问受保护商场API是采用自定义`X-Librecmall-Token`头部；
> 而访问受保护管理后台API则是自定义`X-Librecmall-Admin-Token`头部。
